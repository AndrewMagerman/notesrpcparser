...
public class Line {
	...
	Pattern patternLookifEndsWithBytesInBytesout = Pattern
		.compile("^(.+) \\[(\\d+)\\+(\\d+)=(\\d+)\\](?: \\((.*)\\))??$");
	...
	Pattern ViewOpen = Pattern
		.compile("^.*REP([A-F0-9]{8}):([A-F0-9]{8})-NT([A-F0-9]{8}),(\\d{4}),(\\d{4}).*$");
	...
	Pattern OpenNote = Pattern
		.compile("^.*REP([A-F0-9]{8}):([A-F0-9]{8})-NT([A-F0-9]{8}),([A-F0-9]{8}).*$");
	...
	Pattern UpdateFilters = Pattern
		.compile("^.*REP([A-F0-9]{8}):([A-F0-9]{8})-NT([A-F0-9]{8}).*$");
	...
	Pattern Read_Object = Pattern
		.compile("^.*REP([A-F0-9]{8}):([A-F0-9]{8})-RRV([A-F0-9]{8}),0x([A-F0-9]+) at .*$");
	...
	Pattern Read_Entries = Pattern
		.compile("^.*REP([A-F0-9]{8}):([A-F0-9]{8})-NT([A-F0-9]{8}),Since:.*$");
	...
	Pattern patternReplicaIDpat = Pattern
		.compile("^.*REP([A-F0-9]{8}):([A-F0-9]{8}).*$");
	...
	// positives
	Pattern findserverandFileName = Pattern
		.compile("^.+?(?:OPEN_DB|POLL_DEL_SEQNUM)\\((CN=.+?)!!(.+?\\.n\\w\\w)\\).*$");
	...
	// ms)(Authenticate:
	Pattern p = Pattern
		.compile("^.+?(?:OPEN_DB|POLL_DEL_SEQNUM)\\((.+?)!!(.+?\\.n\\w\\w)\\).*$");
	...
	Pattern SequenceNumberExtract = Pattern
		.compile("^\\((\\d+)-(\\d+) \\[(\\d+)\\]\\) (.+)$");
	...
	return String.format("notes://%s/%s/0/%s?OpenDocument",
		this.getServerName(), this.getDBReplicaID(), this.getNote().getUNID());
    }
...
//	REPC12575D0:001AD0CD-$profile_006config_cn=andrew magerman/ou=magerman/o=notesnet,00400020
	...
	Pattern profileDocument = Pattern
		.compile(".*-\\$profile_\\d{0,3}(.*?)(?:_cn=(.*?))?,\\d{8}");
    ...
    /**
     * Gets the translated command.Translates the command into something
     * digestible. As a default, translate gives the original command back.
     * 
...
//		TODO
		...
		translation = "Loads " + metanote.getNoteType() + " '"
			+ metanote.getNoteTitle() + "'";
		...
		translation = "Writes into this note: "
			+ metanote.getNoteTitle();
	    } else if (command.equals("READ_OBJECT")) {
		translation = "Reads from this this note: "
			+ metanote.getNoteTitle();
    ...
    /**
     * Analyses a single line of the text file and tries to find out things like
     * the sequence number, the time, etc.
     */
	...
	return "Line [LineCount=" + lineCount + ", metanote=" + metanote
		+ ", originalLine=" + originalLine + ", parsedDBFilePath="
		+ parsedDBFilePath + ", parsedDBReplicaID=" + parsedDBReplicaID
		+ ", parsedServerName=" + parsedServerName + "]";
    }
